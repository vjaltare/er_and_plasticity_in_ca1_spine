...
class ca1_spine:
    ...
    def __init__(self, g_N, nRyR, n_ip3, Vsoce_0, tau_refill_0, Vspine, scDep):
        ...
        self.Vsoc = self.Vsoc_0 * (0.49/self.d_spine)
    ...
    def spine_model(self, x , t):
        ...
        if self.struct_plas_flag:
            ...
            if self.ip3r_tot == 0 and self.ryr_tot == 0:
                Vsoc = 0
            else:
                Vsoc = self.Vsoc_0*(0.49/self.d_spine)
        ...
        IP3_eq = self.k_ip3 * (0.49/self.d_spine) * PLCact + dip3  #hill(Gact) 
...
#         ca_eq = (-self.b1f*ca*PLC_PIP2 - self.b2f*ca*Gact_PLC_PIP2 - 1111*IP3K*ca*ca - 1111*IP3K*ca*ca + (self.b1b*ca_PLC_PIP2 + self.b2b*ca_Gact_PLC_PIP2 + 100*IP3K_2ca+100*IP3K_2ca)) 
        ...
        Inmda = sr * self.g_N * (u - self.E_N) /(1.0 + 0.28 * np.exp(-0.062 * u))
        
        if self.input_pattern == "stdp":
            sp_hh_eq = -(1/self.Cmem) * ( self.g_L*(u - self.E_L) + (self.I_A(self.flag,u,t)/self.Aspine) + (Inmda/self.Aspine) \
                                         - (self.gc/self.Aspine)*(ud - u) - I_vgcc/self.Aspine)
...
#     rhov0,rhovp0 = [rho_0*pp0/(pp0 + PK(0,c40)), rho_0*PK(0,c40)/(pp0 + PK(0,c40))]
    ...
    Rneck = 1e9  ## Ohm -> changed to 1 GOhm 01/06/21
    gc = 1.0/Rneck ## S
    ...
    def get_resting_params(self):
        ...
        print("initial sr = {}".format(sol0[-1,-11]))
        print("Vspine={}".format(0.001 + 0.24*sol0[-1,-12]))
...
#         return sp_train
    ...
    def sc_inputs(self, beta_pre, beta_post, tmax, f_burst_min, f_burst_max, max_inputs):
        """emulates the realistic Schaffer-Collateral place cell firing pattern as described in Issac et al. J.Neurosci 2009
        params: beta_pre, beta_post, tmax, f_burst_min, f_burst_max, max_inputs
        ...
        t_burst_pre = get_tBurst(beta_pre, tmax)
        t_burst_post = get_tBurst(beta_post, tmax)
        ...
        t = np.arange(t_min, t_max + 1., 0.001)#np.linspace(t_min, t_max, n_points)
        
        stdp_sol = odeint(self.spine_model, xinit, t, rtol = rtol, atol = atol)
        print(f"Time taken = {time() - tt} s")
        
        return stdp_sol, t
    
    def pre_place_fld_bursts(self, beta_pre, tmax, f_burst_min, f_burst_max, max_inputs):
        tt = time()
        ...
        t_burst_pre = get_tBurst(beta_pre, tmax)
        self.tpre = get_ISI(t_burst_pre, f_burst_min, f_burst_max, max_inputs)
        t_max = np.amax(self.tpre)
        t_min = np.amin(self.tpre)
        print(f"tmin = {t_min: .02f}, tmax = {t_max: .02f}")
        #print(f"tBpre = {t_burst_pre}, tBpost = {t_burst_post}")
        print(f"tpre = {self.tpre}")
    ...
    def paired_bursts_analysis(self, f_spiking, t_ibi):
        tt = time()
        ...
        self.tpre = paired_burst(f_spiking, t_ibi)
        t_max = np.amax(self.tpre)
        t_min = np.amin(self.tpre)
        print(f"tmin = {t_min: .02f}, tmax = {t_max: .02f}")
        #print(f"tBpre = {t_burst_pre}, tBpost = {t_burst_post}")
    ...
    def nmda_flux2(self, sol):
        return [-(self.g_N_Ca/self.Vspine) * (1/(1.0 + 0.28 * np.exp(-0.062 * u))) * 0.078 * u * (ca - self.ca_ext*np.exp(-0.078*u))/(1 - np.exp(-0.078*u)) for u,ca in zip(sol[:,-5],sol[:,-1])]
