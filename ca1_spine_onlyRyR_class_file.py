...
######################################### RDP ################################################################
    ...
    def do_rdp(self, f_input, n_inputs):
        ...
        self.flag = 1
        self.tpre = [i/float(f_input) for i in range(int(n_inputs))]
    ...
    def theta_burst(self, lam, mu, sd, n_turns):
        ...
        params: lam: mean no. of inputs from poisson distr; mu, sd: mean and SD of normal distr, used to sample ISI; n_turns: no. of times the stim is applied. This function saves the time course data in a csv file"""
        seed(234534)
