...
#         ca_eq = (-self.b1f*ca*PLC_PIP2 - self.b2f*ca*Gact_PLC_PIP2 - 1111*IP3K*ca*ca - 1111*IP3K*ca*ca + (self.b1b*ca_PLC_PIP2 + self.b2b*ca_Gact_PLC_PIP2 + 100*IP3K_2ca+100*IP3K_2ca)) 
    ...
    #         dend_hh_eq = 0
    #     else: 
        sp_hh_eq = 0.#-(1/self.Cmem) * ( self.g_L*(u - self.E_L) + (self.I_A(self.flag,u,t)/self.Aspine) + (Inmda/self.Aspine) - (self.gc/self.Aspine)*(ud - u) - I_vgcc/self.Aspine)
            #sp_hh_eq = -(1/Cmem) * ( g_L*(u - E_L) + I_A(s,u,t)/Aspine + I_N(s,u,t)/Aspine - (gc/Aspine)*(ud - u) - I_vgcc/Aspine)
        dend_hh_eq = 0.#-(1/self.Cmem) * ( self.g_L*(ud - self.E_L) + self.rho_spines*self.gc*(ud - u))
        ...
        return [Ract_eq,Gact_eq,PLCact_eq,IP3_eq,IP3K_eq,IP3K_2ca_eq,IP5P_eq] + ryr_eq + [pH_eq, pL_eq, cbp_eq, Bslow_eq] + calb_eqs + cam_eqs + [erB1_eq, erB2_eq] + [sr_eq, ir_eq] + [Psoc_eq] + [mv_eq, hv_eq] + [w_eq] + [sp_hh_eq, dend_hh_eq,\
                h_eq, ca_er_eq, ca_eq]
