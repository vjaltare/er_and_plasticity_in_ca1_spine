...
class ca1_spine:
    ...
    def __init__(self, g_N, nRyR, n_ip3, tau_refill, Vspine): #add ksk if required
        """initialize the spine parameters and volume/surface area-dependent constants: 
        params: (nmda conductance (pS), #ryr, #ip4rs, tau_refill (s), Vspine(um^3))"""
        self.g_N = float(g_N) * 1e-12 #S
        self.ryr_tot = int(nRyR)
        self.ip3_tot = int(n_ip3)
    ...
    ############################################ SOCC Model ##########################################
    ############################################################### Defining parameters ######################################################
    V_socc_max = 1e3 ## changed on 2.3.21  #1.57 #um/s per channel max flux
    K_socc = 50 #um/s
    tau_socc = 0.1 #s #30 
    ...
    ## Resting Ca in ER (uM):
    ca_er_0 = 250#3e3#460##1.182e4#250 
    ...
    cam_tot = 50
#     g_N_Ca = 0.1 * (self.g_N/(2*self.F*78.0*self.ca_ext)) * 1e6   ## Ca conductance of NMDAR channels; liter/sec
#     k_erleak = self.Vmax_serca * (self.ca_0**2)/((self.Kd_serca**2 + self.ca_0**2)*(self.ca_er_0 - self.ca_0))  ## /s
# #     .g_N_Ca = 0.1 * (g_N/(2*self.F*78.0*self.ca_ext)) * 1e6   ## Ca conductance of NMDAR channels; liter/sec
#     self.k_erleak = self.Vmax_serca * (self.ca_0**2)/((self.Kd_serca**2 + self.ca_0**2)*(self.ca_er_0 - self.ca_0))  ## /s
    ...
    #################### Rate Constants ###################################
        ### defining reaction constants from Saftenku ##
...
##### Coupled ODEs describing ER bearing spine head ###################
    def spine_model(self,x,t, vsp, ga):
        ...
        cbp, Bslow, calb, calb_m1, calb_h1, calb_m2, calb_h2, calb_m1h1, calb_m2h1, calb_m1h2, c1n0, c2n0, c0n1, c0n2, c1n1, c2n1, c1n2, c2n2, R_Gq, Gact,IP3,ca_Gact_PLC_PIP2,DAGdegr,\
        ...
        ca_er, ca, ryrC1, ryrC2, ryrC3, ryrC4, ryrO1, ryrO2, ryrO3, Bcer, Bper, p_socc = x
        ###########|----------- RyR ------------------------------|   |-Buffer-|  |-socc-|##############
        ...
        d_spine = np.power((aspine/np.pi), 1/2) * 1e4 #um
        #print(w)
...
#         ca_er_eq = - self.nRyR * Jrel  + (self.ca_er_0 - ca_er)/self.tau_refill
        ...
        IP3_eq = +self.b6*ca_PLC_PIP2 + self.b7*ca_Gact_PLC_PIP2 - 100*IP3K_2ca*IP3 + 80*IP3_IP3K_2ca - 12.5*IP5P*IP3 + 72*IP3_IP5P
        ...
        ca_er_eq = -self.alpha_ip3r * self.ip3r_tot * ((x*y*h)**3) * (ca_er - ca)/(self.Nav * vsp * 0.1) + (self.ca_er_0 - ca_er)/self.tau_refill## vsp * 0.1 = ver
        ...
        ## New Pumps ##
        ## Extrusion kinetics:
        ca_eq += -(6.0/d_spine)*200*5*(ca/(ca+20.0) - self.ca_0/(self.ca_0+20.0))  ## Low-aff pump 20.0
        ca_eq += -(6.0/d_spine)*200*0.5*(ca/(ca+0.5) - self.ca_0/(self.ca_0+0.5))  ## High-aff pump
...
#         pL_eq = self.k3L*(self.pLtot - pL) - ca*pL*self.k1L + self.k2L*(self.pLtot - pL)
        ...
        ryr_eqs =  [ryrC1_eq, ryrC2_eq, ryrC3_eq, ryrC4_eq, ryrO1_eq, ryrO2_eq, ryrO3_eq]
        
        ca_eq += self.ryr_tot * (ryrO1+ryrO2+ryrO3) * self.alpha_ryr * (ca_er - ca)/(self.Nav * vsp)

        ca_er_eq += -self.ryr_tot * (ryrO1+ryrO2+ryrO3) * self.alpha_ryr * (ca_er - ca)/(self.Nav * vsp * 0.1)

        rho_ryr = (1e6)*self.ryr_tot/(self.Nav * vsp)
...
#                      -(self.ko2c4 + self.ko2c5)*o2 + self.kc4o2*c4 + self.kc5o2*c5]
        ...
        return [cbp_eq, Bslow_eq] + calb_eqs + cam_eqs + [R_Gq_eq,Gact_eq,\
    ...
    def get_resting_params(self):
        ...
        xinit0 = mGluR_init + buff_init + CaM_init + vgcc_init + w_init + voltage_init + ip3r_init + ca_init + RyR_init + calreticulin_init + SOCC_init 
        ...
        print("initial ER calcium = {}".format(sol0[-1,-12]))
        print("initial cyto calcium = {}".format(sol0[-1,-11]))
